...
client.getFeed = function (url, params, callback) {
  ...
  request.doRequest(url, params, function (err, body) {
    ...
    if (body.statusCode === 401) {
      return client.refreshToken(function(err, result) {
        if(!err && result && !result.error && result.access_token) {
          ...
          fs.writeFile(path.join(__dirname, 'token.json'), JSON.stringify(TOKEN, null, 4));
          params.qs.access_token = TOKEN.access_token;
          request.doRequest(url, params, function(err, body){
